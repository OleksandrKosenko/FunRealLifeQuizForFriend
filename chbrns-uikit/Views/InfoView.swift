...
protocol PopUpable: UIView {
    var lottieAnimationView: LottieAnimationView! { get set }
    var viewLabel: UILabel! { get set }
    var nameOfLottieAnimation: String! { get set }
}
...
extension PopUpable where Self: UIView {
    ...
    func setupNameAnimation(name: String) {
        nameOfLottieAnimation = name
    }
}

class InfoView: UIView, PopUpable {
    ...
    func setupInfoAnimationView() {
        ...
        lottieAnimationView = .init(name: "Warning")
        lottieAnimationView.frame = CGRect(x: (frame.size.width - 300) / 2, y: (frame.size.height - 360), width: 300, height: 125)
        ...
        //        lottieAnimationInfoView.loopMode = .loop
...
//        lottieAnimationView.play()
    ...
    func setupInfoViewLabal() {
        viewLabel = UILabel(frame: CGRect(x: (frame.size.width - 250) / 2, y: (frame.size.height - 245), width: 260, height: 145))
        viewLabel.backgroundColor = .clear
        viewLabel.textColor = .white
        viewLabel.font = UIFont(name: "Jura-Medium", size: 20)
    ...
    func setupWarningButton() {
        viewButton = UIButton(frame: CGRect(x: (frame.size.width - 100) / 2, y: (frame.size.height - 90), width: 100, height: 50))
        viewButton.backgroundColor = UIColor(named: "customGray")?.withAlphaComponent(0.6)
        viewButton.setTitle("OK", for: .normal)
